...
namespace Mermaid { namespace Base
{
    ...
    typedef struct AssetInfo
    {
        AssetInfo
        (
            std::string name, std::string author="", std::string copyright="", std::string license="", std::string description=""
        ): 
        Name(name), Author(author), Copyright(copyright), License(license), Description(description){};
